...
exports.UpdateSubService=function(req,res){
  (async()=>{
      const updatesubservicedata=req.body;
      const subserviceid=req.params.id;
      let dateobj=new Date();
      const updateservice=await client.query('update SubServices set sub_servicename=$1,price=$2,short_description=$3,long_description=$4,time_duration=$5,modified_date=$6 where subservice_id=$7',[updatesubservicedata.sub_servicename,updatesubservicedata.price,updatesubservicedata.short_description,updatesubservicedata.long_description,updatesubservicedata.time_duration,dateobj.getDate()+'-'+(dateobj.getMonth()+1)+'-'+dateobj.getFullYear(),subserviceid],(error)=>{
        if(error){
          return error;
        }
        res.status(200).json({
